...
int CBitmapImage::ScrollBitmap(int dx, int dy)
{
	...
	if(dx!=0)
	{
		...
		if(dx>0)
		{//向右翻转
			temImage.CreateBitmapImage(0,0,dx,height,bpp);
			temImage.CopyBitmapImage(0,0,*this,width-dx,0,dx,height);
			UCHAR* sourAddr=buffer;
			int shift=(bpp>>3)*dx;
			for(int i=0; i<height; i++)
			{
				memmove(sourAddr+shift,sourAddr,(width-dx)*(bpp>>3));
				sourAddr+=width*(bpp>>3);
			}
		...
		{//向左翻转
			...
			for(int i=0; i<height; i++)
			{
				memmove(sourAddr,sourAddr+shift,(width-dx)*(bpp>>3));
				sourAddr+=width*(bpp>>3);
			}
